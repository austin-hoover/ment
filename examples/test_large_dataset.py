"""Test 2D reconstruction with large number of projections."""
import argparse
import os
import pathlib
import numpy as np
...
def main(args: argparse.Namespace) -> None:
    ...
    for transform in transforms:
        diagnostic = ment.diag.Histogram1D(axis=0, edges=bin_edges, thresh=0.001, thresh_type="frac")
    ...
    prior = ment.GaussianPrior(ndim=2, scale=[5.0, 5.0])
    ...
    integration_limits = [[(-4.0, 4.0) for _ in diagnostics] for _ in transforms]
    ...
    def plot_projections(model: ment.MENT):
        ...
        nplot = len(projections_true)
        ncols = min(nplot, 10)
        nrows = int(np.ceil(nplot / ncols))
        figwidth = 1.5 * ncols
        figwidth = min(figwidth, 12.0)
        figheight = 0.80 * nrows
    ...
    for epoch in range(args.epochs):
        ...
        if epoch == args.epochs - 1:
            fig, ax = plot_projections(model)
            fig.savefig(os.path.join(output_dir, f"fig_proj_{epoch:02.0f}.png"), dpi=300.0)
            plt.close("all")
    ...
    for ax, x in zip(axs, [x_pred, x_true]):
        ax.hist2d(x[:, 0], x[:, 1], bins=85, range=[(-4.0, 4.0), (-4.0, 4.0)])
        ax.set_aspect(1.0)
    fig.savefig(os.path.join(output_dir, f"fig_dist_{epoch:02.0f}.png"), dpi=300.0)
    plt.close("all")
