...
def nurs_vectorized(
    ...
    log_prob_func: accepts (num_chains, num_dim) -> (num_chains,)
    ...
    def stopping_condition(tree):
        # tree = (theta, lp, theta_min, theta_max, lp_min, lp_max)
        log_epsilon = log_threshold + log_step_size + tree[1]
        return (tree[4] < log_epsilon) & (tree[5] < log_epsilon)

    def leaf(theta: np.ndarray):
        # theta: (num_chains, num_dim)
        lp = log_prob_func(theta)
        return (theta, lp, theta, theta, lp, lp)

    def combine_trees(tree1, tree2, direction):
        # direction: (num_chains,)
        lp1, lp2 = tree1[1], tree2[1]
        lp12 = logsumexp(np.stack([lp1, lp2], axis=0), axis=0)
        probs = np.exp(lp2 - lp12)
        update = rng.random(num_chains) < probs
        selected = np.where(update[:, None], tree2[0], tree1[0])

        cond = direction == 1
        left_theta_min = np.where(cond[:, None], tree1[2], tree2[2])
        right_theta_max = np.where(cond[:, None], tree2[3], tree1[3])
        left_lp_min = np.where(cond, tree1[4], tree2[4])
        right_lp_max = np.where(cond, tree2[5], tree1[5])
    ...
    def build_tree(depth, theta_last, rho, direction):
        """Recursive tree construction (vectorized)."""
        h = step_size * (2 * direction - 1)  # (num_chains,)
        if depth == 0:
        ...
        theta_mid = np.where((direction == 1)[:, None], tree1[3], tree1[2])
        tree2 = build_tree(depth - 1, theta_mid, rho, direction)
        if tree2 is None:
            return None
    ...
    def metropolis(theta, rho):
        ...
        s = (rng.random(num_chains) - 0.5) * step_size
    ...
    def transition(theta):
        ...
        for tree_depth in range(max_doublings):
            direction = directions[:, tree_depth]
            theta_mid = np.where((direction == 1)[:, None], tree[3], tree[2])
            tree_next = build_tree(tree_depth, theta_mid, rho, direction)
    ...
    def sample():
        ...
        depths = np.zeros((num_draws, num_chains), dtype=int)
